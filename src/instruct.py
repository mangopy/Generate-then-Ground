...
<TThought> what is Jon Wertheim's nationality?
...
BOOL_GENERATION_USER=""""You need to decompose a complex question into several sub-questions step by step and give an answer to each sub-query, until finishing it with a final answer.
Specifically, please take care of your output format:
1. Each decomposed sub-query should start with a <Thought>.
2. Each answer should start with a <Answer>.
3. The final answer should be wrapped with FINISH[Yes/No].
